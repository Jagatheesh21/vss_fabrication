...
@section('content')
  ...
  @endif
    ...
    <div class="card ">
        ...
        <div class="card-body">
            <div class="col-md-12">
                <form id="category_save" method="POST" action="{{route('po_master.update',$po_master->id)}}">
                  @csrf
                  @method('PUT')
                    <div class="row mb-3">
                      <div class="col-md-4">
                        <label for="purchase_order_number" class="col-sm-12 col-form-label required">Purchase Order Number*</label>
                        <input type="text" class="form-control" name="rm_po_number" readonly value="{{$po_master->rm_po_number}}">
                      </div>
                      <div class="col-md-4">
                        ...
                        <select name="supplier_id" id="supplier_id" class="form-control select2">
                          ...
                          <option value="{{$supplier->id}}" @if ($supplier->id==$po_master->supplier_id) selected @endif>{{$supplier->code}}</option>
                          @endforeach
                        </select>
                        @error('supplier_id')
                      ...
                      <div class="col-md-4">
                        ...
                        <select name="type_id" id="type_id" class="form-control select2">
                          ...
                          @foreach ($types as $type)
                              <option value="{{$type->id}}" @if ($type->id==$po_master->raw_material->type_id) selected @endif>{{$type->name}}</option>
                          @endforeach
                        </select>
                        @error('type_id')
                        <span class="text-danger">{{$message}}</span>
                      ...
                      <div class="col-md-4">
                        ...
                        <select name="raw_material_id" id="raw_material_id" class="form-control select2">
                          ...
                          @foreach ($raw_materials as $raw_material)
                              <option value="{{ $raw_material->id }}" @if ($raw_material->id==$po_master->raw_material_id) selected @endif>{{ $raw_material->name }}</option>
                          @endforeach
                      ...
                      <div class="col-md-3">
                        ...
                        <input type="date" name="po_date" id="po_date" class="form-control" value="{{ $po_master->po_date }}">
                        @error('po_date')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      <div class="col-md-3">
                        ...
                        <input type="text" name="invoice_number" id="invoice_number" class="form-control" value="{{ $po_master->invoice_number }}">
                    ...
                    <div class="row mb-3">
                      <div class="col-md-3">
                        ...
                        <input type="number" name="po_quantity" id="quantity" class="form-control" min="1" value="{{ $po_master->po_quantity }}">
                        @error('po_quantity')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      ...
                      <div class="col-md-3">
                        ...
                        <select name="uom_id"  class="form-control" id="uom_id">
                          ...
                          @foreach ($uoms as $uom)
                              <option value="{{ $uom->id }}" @if ($uom->id==$po_master->uom_id) selected @endif>{{ $uom->name }}</option>
                          @endforeach
                        </select>
                        @error('uom_id')
                        <span class="text-danger">{{$message}}</span>
                      ...
                      <div class="col-md-3">
                        ...
                        <input type="number" name="material_quantity" id="material_quantity" class="form-control" value="{{ $po_master->material_quantity }}" min="1">
                      ...
                      <div class="col-md-3">
                        ...
                        <select name="material_uom_id"  class="form-control" id="material_uom_id">
                          ...
                          @foreach ($uoms as $uom)
                              <option value="{{ $uom->id }}" @if ($uom->id==$po_master->material_uom_id) selected @endif>{{ $uom->name }}</option>
                          @endforeach
                        </select>
                      ...
                      <div class="col-md-3">
                        ...
                        <input type="text" name="unit_material_quantity" id="unit_material_quantity" class="form-control" value="{{ $po_master->unit_material_quantity }}" min="1" readonly>
                        @error('material_uom_id')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                   ...
                   <input type="hidden" name="remarks" value="PO">
                    <button type="submit" id="submit" class="btn btn-primary" onclick="return confirm('Are you sure you want to Update?')">Update</button>
                  </form>
            </div>
        </div>
    </div>
